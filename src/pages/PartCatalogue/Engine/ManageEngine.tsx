...
import { useEngine } from "@/hooks/usePartCatalogue";
...
export default function ManageEngine() {
    ...
    const handleAddEngine = () => {
        navigate('/epc/engine/create');
    };

    const handleEditEngine = (engine: Engine) => {
        navigate(`/epc/engine/edit/${engine.engines_id}`);
    };

    const handleDeleteWithConfirmation = async (engine: Engine) => {
        const confirmed = await confirmation.showConfirmation({
            title: 'Delete Engine',
            message: `Are you sure you want to delete "${engine.engines_name_en}"? This action cannot be undone.`,
            confirmText: 'Delete',
            type: 'danger'
        });

        if (confirmed) {
            await handleDeleteEngine(engine.engines_id);
        }
    };

    const handlePageChange = (page: number) => {
        fetchEngines(page, pagination?.limit || 10);
    };

    const engineColumns: TableColumn<Engine>[] = [
        {
            name: 'Engine Name (EN)',
            selector: row => row.engines_name_en || '-',
        },
        {
            name: 'Engine Name (CN)',
            selector: row => row.engines_name_cn || '-',
        },
        {
            name: 'Description',
            selector: row => row.engines_description || '-',
            width: '550px',
            wrap: true,
        },
        {
            name: 'Type Engines',
            selector: row => row.type_engines?.length || 0,
            center: true,
            cell: (row: Engine) => {
                console.log({
                    row,
                    type: row.type_engines
                });
                
                return(
                <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs">
                    {row.type_engines?.length || 0} types
                </span>
            )}
        },
        createActionsColumn([
                    ...
                    <div className="flex justify-between items-center">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-gray-500">
                                Manage system engines and their configurations
                            </p>
                        </div>
                ...
                <div className="p-6 font-secondary">
                    <CustomDataTable
                        ...
                        onChangeRowsPerPage={(newPerPage) => {
                            // Fetch engines with new per_page limit and reset to page 1
                            fetchEngines(1, newPerPage);
                        }}
                        responsive
                        highlightOnHover
                        striped={false}
