...
import { useCabin } from "@/hooks/usePartCatalogue";
...
export default function ManageCabin() {
    ...
    const handleAddCabin = () => {
        navigate('/epc/cabin/create');
    };

    const handleEditCabin = (cabin: Cabin) => {
        navigate(`/epc/cabin/edit/${cabin.cabines_id}`);
    };

    const handleDeleteWithConfirmation = async (cabin: Cabin) => {
        const confirmed = await confirmation.showConfirmation({
            title: 'Delete Cabin',
            message: `Are you sure you want to delete "${cabin.cabines_name_en}"? This action cannot be undone.`,
            confirmText: 'Delete',
            type: 'danger'
        });

        if (confirmed) {
            await handleDeleteCabin(cabin.cabines_id);
        }
    };

    const handlePageChange = (page: number) => {
        fetchCabins(page, pagination?.limit || 10);
    };

    const cabinColumns: TableColumn<Cabin>[] = [
        {
            name: 'Cabin Name (EN)',
            selector: row => row.cabines_name_en || '-',
        },
        {
            name: 'Cabin Name (CN)',
            selector: row => row.cabines_name_cn || '-',
        },
        {
            name: 'Description',
            selector: row => row.cabines_description || '-',
            width: '550px',
            wrap: true,
        },
        {
            name: 'Type Cabines',
            selector: row => row.type_cabines?.length || 0,
            center: true,
            cell: (row: Cabin) => (
                <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs">
                    {row.type_cabines?.length || 0} types
                </span>
            )
        },
        createActionsColumn([
                    ...
                    <div className="flex justify-between items-center">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-gray-500">
                                Manage system cabins and their configurations
                            </p>
                        </div>
                ...
                <div className="p-6 font-secondary">
                    <CustomDataTable
                        ...
                        onChangeRowsPerPage={(newPerPage) => {
                            // Fetch cabins with new per_page limit and reset to page 1
                            fetchCabins(1, newPerPage);
                        }}
                        responsive
                        highlightOnHover
                        striped={false}
